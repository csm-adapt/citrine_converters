...
def converter(files=[], **kwds):
    ...
    if epsilon['strain'].values.mean() < 0:
        epsilon['strain'].values *= -1
    if sigma['stress'].values.mean() < 0:
        sigma['stress'].values *= -1
    ...
    mechprop = MechanicalProperties(epsilon, sigma, interactive=interactive)
