...
from ..tools.interactive import DraggableLine
from ..tools.interactive import trim
from ..tools import (
    linear_merge,
    Normalized,
    resample,
...
class MechanicalProperties(object):
    ...
    def __init__(self, epsilon, sigma, interactive=False):
        ...
        if interactive:
            mask = trim(strain, stress, c='b', s=10)
            time = time[mask].copy()
...
def interactive_approximator(mechprop, **kwds) :
    ...
    ax.set_title("Press enter to accept the selected points.")
    plt.show(block=True)
    # process the data
    xlo, ylo = dl.first
    xhi, yhi = dl.second
